...
public class DistributedExecutorService {
    ...
    public <T> Future<T> executeTaskOnMemberWithProfile(Callable<T> taskCallable, String profile) {
        ...
        var instancesWithMatchingProfile = instances.stream()
            .filter(instance -> Arrays.asList(instance.getMetadata().getOrDefault("profile", "").split(",")).contains(profile)).toList();
        ...
        log.info("Hazelcast members are {}", hazelcastInstance.getCluster().getMembers().stream().map(m -> "Address: %s, Attributes %s".formatted(m.getAddress(), m.getAttributes().toString())).collect(Collectors.toList()));
